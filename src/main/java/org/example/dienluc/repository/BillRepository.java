...
public interface BillRepository extends JpaRepository<Bill, Integer> {
    ...
    @Query("SELECT b from Bill b where b.status = ?2 and b.electricRecording.powerMeter.id IN (SELECT c.powerMeter.id from Contract c WHERE c.client.id = ?1)")
    public List<Bill> findByClientIdAndStatus(Integer clientId, Boolean status);

    @Query("SELECT b from Bill b  where b.electricRecording.powerMeter.id IN (SELECT c.powerMeter.id from Contract c WHERE c.client.id = ?1)")
    ...
    @Query(value = "SELECT YEAR(NGAYHANTT) as year , MONTH(NGAYHANTT) as month, TONGTIEN as amount FROM HOADON WHERE IDGHIDIEN IN \n" +
            "(SELECT ID FROM GHIDIEN WHERE IDDONGHODIEN IN (SELECT IDDONGHODIEN FROM HOPDONG WHERE IDKHACHHANG = ?1))\n" +
